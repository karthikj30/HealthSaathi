...
app.get("/api/documents", getAuthUser, (req, res) => {
  pool.query("SELECT id, title, notes, file_name, file_mime, file_data, created_at FROM documents WHERE user_email = $1 ORDER BY created_at DESC", [req.authUser.email], (err, result) => {
    if (err) {
      return res.status(500).json({ error: "Database error" });
    }
...
app.post("/api/documents", getAuthUser, (req, res) => {
  ...
  pool.query('INSERT INTO documents (user_email, title, notes, file_name, file_mime, file_data, created_at) VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING id', 
    [req.authUser.email, resolvedTitle, notes || "No notes", fileName || null, fileMime || null, fileData || null, createdAt], (err, result) => {
    if (err) {
      return res.status(500).json({ error: "Database error" });
    }
    res.status(201).json({ ok: true, id: result.rows[0].id });
